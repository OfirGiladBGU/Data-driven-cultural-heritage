...
#Function to resample point cloud to have exactly n points
...
#Function to process the point cloud files
def processPCD(network, opt):
    ...
    parts_path = os.path.join(opt.outputFolder, "..", "parts_3d")
    os.makedirs(parts_path, exist_ok=True)

    #The resulting geometry is stored as point clouds in disk
    save_point_cloud(os.path.join(parts_path, opt.object + '_partial.pcd'), partial)
    save_point_cloud(os.path.join(parts_path, opt.object + '_hole.pcd'), hole)
